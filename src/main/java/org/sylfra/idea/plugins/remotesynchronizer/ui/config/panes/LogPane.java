...
public final class LogPane extends JPanel implements IConfigPane
{
  ...
  public void buildUI(ConfigPathsManager pathsManager)
  {
    ckClearBeforeSynchro =
      new JBCheckBox(LabelsFactory.get(LabelsFactory.LB_CLEAR_BEFORE_SYNCHRO));
    ckAutoPopup =
      new JBCheckBox(LabelsFactory.get(LabelsFactory.LB_AUTO_POPUP_LOGS));
  ...
  {
    cbFontFamily = new ComboBox<>(FONT_NAMES);
  ...
  {
    ckLogSrcPaths =
      new JBCheckBox(LabelsFactory.get(LabelsFactory.LB_SHOW_SOURCE_PATHS));
    ckLogExludedPaths =
      new JBCheckBox(LabelsFactory.get(LabelsFactory.LB_SHOW_EXCLUDED_PATHS));
    ckLogIdenticalPaths =
      new JBCheckBox(LabelsFactory.get(LabelsFactory.LB_SHOW_IDENTICAL_PATHS));
